import {
  APEX_PATH,
  CARY_PATH,
  FUQUAY_VARINA_PATH,
  GARNER_PATH,
  HOLLY_SPRINGS_PATH,
  HOME_PATH,
  KNIGHTDALE_PATH,
  MORRISVILLE_PATH
} from "./constants";

export const primaryStyle = "text-white bg-blue hover:bg-dark-blue border-blue";
export const secondaryStyle =
  "text-black bg-gray hover:bg-dark-gray hover:text-white border-gray hover:border-white";

export const localAreas = [
  "apex",
  "cary",
  "garner",
  "holly-springs",
  "knightdale",
  "morrisville",
  "/raleigh",
  "fuquay-varina",
];

export const locations = [
  "Apex",
  "Cary",
  "Garner",
  "Holly Springs",
  "Knightdale",
  "Morrisville",
  "Raleigh",
  "Fuquay Varina",
];

export const locationLinks = [
  APEX_PATH,
  CARY_PATH,
  GARNER_PATH,
  HOLLY_SPRINGS_PATH,
  KNIGHTDALE_PATH,
  MORRISVILLE_PATH,
  HOME_PATH,
  FUQUAY_VARINA_PATH,
];

export const footerLinks = [
  "Mobile Detailing",
  "Paint Correction",
  "Ceramic Coating",
  "PPF",
];

export const footerLinksActual = [
  "services",
  "services",
  "services",
  "services",
];

export const servicesLinks = ["Services", "Booking", "Faq", "About"];
export const servicesLinksActual = ["services", "booking", "faq", "about"];

export const businessAddress = "Apex, NC 27502";
export const businessHours = ["MON-SUN 9AM-8PM"];

export const businessName = "Spotless";
export const fullBusinessName = "Spotless Auto Detailing";

export const reviews = [
  {
    name: "John Wrick",
    rating: 5,
    review: "Excellent attention to detail, a job well done.",
  },
  {
    name: "Sean Tinney",
    rating: 5,
    review:
      "Car smelled great, wax job was really smooth and has given the car it's sharp look back. Nik is incredibly easy to work with and a great communicator. Will 100% be using his services again. Really appreciate the work he did. Hands down the guy to go to.",
  },
  {
    name: "Mitch Bates",
    rating: 5,
    review:
      "Absolutely incredible, professional and prompt service, cannot recommend Nik's work enough!",
  },
  {
    name: "Steve Superville",
    rating: 5,
    review:
      "My car has never been more clean since I bought it. A years worth of travel, kids and dirt — I can’t tell it’s not new. Nik has an awesome work ethic. Can’t recommend more highly.",
  },
  {
    name: "Brooke Bertaux",
    rating: 5,
    review:
      "Did a rlly good job cleaning all of it. Very sweet and nice guy. He did a fantastic job getting every crevice. I really appreciate him.",
  },
  {
    name: "Alice Seely",
    rating: 5,
    review:
      "Highly recommend this service! Our vehicle looks brand new and smells great! Thanks!",
  },
  {
    name: "William Nolan",
    rating: 5,
    review:
      "Absolutely incredible, professional and prompt service, cannot recommend Nik's work enough!",
  },
  {
    name: "Chad Lew",
    rating: 5,
    review:
      "Amazing work. We got the car detailed inside and out including carpet shampooing and ceramic coating. My Toyota Highlander looks better than when I bought it from the dealership! He working really hard getting every little crevice and kept me informed the whole way. I would highly recommend Nik.",
  },
  {
    name: "Mujahid Amme",
    rating: 5,
    review:
      "Absolutely incredible, professional and prompt service, cannot recommend Nik's work enough!",
  },
  {
    name: "Anastasia Chernenkaia",
    rating: 5,
    review: "Great service! Highly recommend",
  },
  {
    name: "Leyla Iz",
    rating: 5,
    review:
      "Nik was very professional and attention to detail. My RAV4 looks and feels like a brand new vehicle. Nik spent a half a day detailing this dirty car and NOW look at it…sparkling clean 😎",
  },
  {
    name: "Tommy Fletcher",
    rating: 5,
    review:
      "Nik was very meticulous and detail oriented. I will definitely use him again. Already recommended friends and family.",
  },
  {
    name: "Michael Rowland",
    rating: 5,
    review: "Excellent and timely service",
  },
  {
    name: "Ryan Huang",
    rating: 5,
    review:
      "My SUV has lots of dust and food debris from baby eating accumulated all over the years. Nik made my car just look like a brand new vehicle just bought from a dealership. Amazing job.",
  },
  {
    name: "Kondal Madaram",
    rating: 5,
    review: "Great work, went above and beyond",
  },
  {
    name: "D. Bam",
    rating: 5,
    review:
      "Nik was very meticulous and detail oriented. I will definitely use him again. Already recommended friends and family.",
  },
  {
    name: "Arun",
    rating: 5,
    review:
      "Nik did a fantastic job with car detailing. It was as though a new car was brought home from the dealership. He is extremely professional and you will be very pleased with the overall engagement.",
  },
  {
    name: "Debra Wagner Seehaver",
    rating: 5,
    review: "My car looks great. Nik did a fabulous job!!",
  },
  {
    name: "Eric Strickland",
    rating: 5,
    review:
      "His attention detail was great! Very prompt and professional. Highly recommend his services to anyone in need of a detail.",
  },

  {
    name: "Chenxi Zhang",
    rating: 5,
    review:
      "I was able to book same day interior detailing service with Spotless. Post booking communications were timely and clear. Arrival was on time. While performing the job, Nik did a great job going thru all details not rushing thru the checklist. And result fully matched my expectation. Will recommend to whomever need interior cleaning and detailing on their vehicles.",
  },
  {
    name: "Asifuddin Syed",
    rating: 5,
    review:
      "Excellent job done. Every small detail was taken care of. I got both interior and exterior done for my Mazda cx5. I wanted the car to look brand new and he delivered. Prompt and professional I highly recommend to anyone looking for car detailing.",
  },
  {
    name: "Steven Sides",
    rating: 5,
    review:
      "Showed up on time and did more than was requested. My truck looks better than when I first took it home. I will be using again and highly recommend.",
  },
  {
    name: "Jared McGrath",
    rating: 5,
    review:
      "Absolutely incredible. Made my truck look identical to when I bought it! I highly recommend!",
  },
  {
    name: "Nancy Yarley",
    rating: 5,
    review:
      "Amazing job!! Nik was extremely thorough and did an amazing job. My car looks brand new again, which is amazing since I have three dogs. I will DEFINITELY call on him again. Highly impressed.",
  },
  {
    name: "Noah Winstead",
    rating: 5,
    review:
      "Nik does an incredible job and is very thorough. He has expertise in solving problems related to cleaning cars and is very communicative regarding any alterations/time-updates required to do a good job. Highly recommend.",
  },
  {
    name: "John King",
    rating: 5,
    review:
      "Have an older vehicle that needed a deep cleaning inside and out. He did an amazing job. Came to my office and worked on the car from 9a till 6p. Great job, took pride in what he did, and very personable.",
  },
  {
    name: "Vanessa Gomez",
    rating: 5,
    review:
      "Just had my car detailed here for the first time,  and can't say enough great things about it. Excellent service, communication, and very detailed. This will be my go to for detailing going forwards.",
  },
  {
    name: "Judith Coetzer",
    rating: 5,
    review:
      "Fantastic detailing done on my car (interior and exterior). Very satisfied and I can recommend this company to everyone",
  },
  {
    name: "Igor Tikhonov",
    rating: 5,
    review:
      "Our detailer arrived on time, went over the package of service and got to work.  His time estimate was accurate and the job was careful and thorough.",
  },
  {
    name: "Hankko Larsen",
    rating: 5,
    review:
      "Just had my car detailed by Spotless Auto Detailing, and I’m thrilled with the results! They were on time, super professional, and paid attention to every little detail. Highly recommend!",
  },
  {
    name: "Natalia Maksimova",
    rating: 5,
    review:
      "I recently bought a used car that is in excellent technical condition but needs good mobile detailing. I am glad I found this high-quality service and fair prices.",
  },
  {
    name: "Jacob Drolshagen",
    rating: 5,
    review:
      "Spotless did an amazing job! My car is cleaner now than when we first bought it. Great service and attention to detail. Highly recommend.",
  },
  {
    name: "Lizet Tikhonov",
    rating: 5,
    review:
      "We got services from Spotless Auto detailing and our van looks like new. I highly recommend their services ! Also, they were great at working on my busy schedule.",
  },
  {
    name: "Radha Maldhure",
    rating: 5,
    review:
      "Great Service. I highly recommend Spotless Auto detailing if you need the convenience of car wash while you’re working at home. My interior and exterior of car is looking awesome clean.",
  },
  {
    name: "Murali Thiagarajan",
    rating: 5,
    review: "Great Service.....!!!",
  },
  {
    name: "Natalia Yáñez Castilo",
    rating: 5,
    review: "Nik and his crew are great! I highly recommend their services!!",
  },
];

export const galleryImages = [
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/0DACF3D9-564F-4817-9266-A08E1BA0AF7F_1719800800%2B(1).webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/112-1687927537-67324accb366f%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/113_1687927537%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/114_1687927538%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/117_1687927941%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/33594FDF-1FC7-4D23-AADA-B6831514F5F6_1686863368%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/5A312470-98CE-4124-A942-1407FD87FE33_1686863363.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/5D4BA67D-7A96-4D27-A2B5-4F2D09F38358_1718500840.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/7E9A8A1F-3448-4E39-82A6-F85B6232E624_1686863361.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/8AD75E48-9635-4933-AE10-3B13265D1F5C_1686863364%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/8D307122-0D42-4EF8-9103-41685B2B8973_1686863365%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/ADD47E3C-CA97-4F68-899D-E407C33DAABC_1686863368%2B720x720.webp ",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/EC5AD31E-2909-47A4-9FE3-DF4D49F50676_1711312087%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/auto-detailing-1536x2048-673251a518bee.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/car-wash-in-boston-massachusetts-67324ad95c26d%2B(1)%2B720x716.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/exterior-car-wash-1536x2048-6732519ccb105.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/interior-car-1536x2048-6732519cc282c.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/interior-detailing-1536x2048-673251a1a1113.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/mobile-car-detailing-bostonmassachusetts-67324ae1c9989%2B720x728.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/porsche-1536x2048-67324ae3b08eb%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/riv_1718500840%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/unnamed-1-1722958239-67324ae7d16c5%2B720x720.webp",
  },
  {
    image:
      "https://pub-a032091c79624619ade299caa8a7dbc6.r2.dev/unnamed_1687394966%2B720x720.webp",
  },
];
